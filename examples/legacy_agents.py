...
@tool
async def get_empresa_id() -> Dict[str, Any]:
    """
    Obtiene el ID de la empresa de las herramientas disponibles.
    """
    return asyncio.run(call_mcp_tool("get_empresa_id", {})) 

@tool
async def get_lista_especialistas() -> Dict[str, Any]:
    """
...
@tool
async def get_especialista_nombre(
    especialista_id: str,
) -> str:
    """
...
1.  **Obtener Datos Iniciales**: Necesitas la siguiente información del usuario. Ve pidiéndola una por una si no la tienes:
    ...
    - Haciendo uso de las herramientas disponibles, obten el ID de la empresa de las herramientas disponibles.
